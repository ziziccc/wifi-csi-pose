...
typedef struct {
    ...
    XFull_pose_accel_uio_map maps[ MAX_UIO_MAPS ];
} XFull_pose_accel_uio_info;

/***************** Variable Definitions **************************************/
static XFull_pose_accel_uio_info uio_info;
...
static int uio_info_read_name(XFull_pose_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFull_pose_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFull_pose_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFull_pose_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFull_pose_accel_Initialize(XFull_pose_accel *InstancePtr, const char* InstanceName) {
	XFull_pose_accel_uio_info *InfoPtr = &uio_info;
