...
// ==============================================================
`timescale 1ns/1ps
module full_pose_accel_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_WEIGHTS_DATA_0    = 6'h1c,
    ADDR_WEIGHTS_DATA_1    = 6'h20,
    ADDR_WEIGHTS_CTRL      = 6'h24,
    ADDR_COMMAND_DATA_0    = 6'h28,
    ADDR_COMMAND_CTRL      = 6'h2c,
    ADDR_FINAL_POSE_DATA_0 = 6'h30,
    ADDR_FINAL_POSE_DATA_1 = 6'h34,
    ADDR_FINAL_POSE_CTRL   = 6'h38,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_FINAL_POSE_DATA_0: begin
                    rdata <= int_final_pose[31:0];
                end
                ADDR_FINAL_POSE_DATA_1: begin
                    rdata <= int_final_pose[63:32];
                end
            endcase
        end
    end
...
assign final_pose        = int_final_pose;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_final_pose[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_FINAL_POSE_DATA_0)
            int_final_pose[31:0] <= (WDATA[31:0] & wmask) | (int_final_pose[31:0] & ~wmask);
    end
end

// int_final_pose[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_final_pose[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_FINAL_POSE_DATA_1)
            int_final_pose[63:32] <= (WDATA[31:0] & wmask) | (int_final_pose[63:32] & ~wmask);
    end
end
