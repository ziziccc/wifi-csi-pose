...
entity full_pose_accel_control_s_axi is
generic (
...
end entity full_pose_accel_control_s_axi;
...
-- 0x24 : reserved
-- 0x28 : Data signal of command
--        bit 31~0 - command[31:0] (Read/Write)
-- 0x2c : reserved
-- 0x30 : Data signal of final_pose
--        bit 31~0 - final_pose[31:0] (Read/Write)
-- 0x34 : Data signal of final_pose
--        bit 31~0 - final_pose[63:32] (Read/Write)
-- 0x38 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of full_pose_accel_control_s_axi is
    ...
    constant ADDR_WEIGHTS_DATA_0    : INTEGER := 16#1c#;
    constant ADDR_WEIGHTS_DATA_1    : INTEGER := 16#20#;
    constant ADDR_WEIGHTS_CTRL      : INTEGER := 16#24#;
    constant ADDR_COMMAND_DATA_0    : INTEGER := 16#28#;
    constant ADDR_COMMAND_CTRL      : INTEGER := 16#2c#;
    constant ADDR_FINAL_POSE_DATA_0 : INTEGER := 16#30#;
    constant ADDR_FINAL_POSE_DATA_1 : INTEGER := 16#34#;
    constant ADDR_FINAL_POSE_CTRL   : INTEGER := 16#38#;
    ...
    signal int_final_pose      : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_FINAL_POSE_DATA_0 =>
                        rdata_data <= RESIZE(int_final_pose(31 downto 0), 32);
                    when ADDR_FINAL_POSE_DATA_1 =>
                        rdata_data <= RESIZE(int_final_pose(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    final_pose           <= STD_LOGIC_VECTOR(int_final_pose);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FINAL_POSE_DATA_0) then
                    int_final_pose(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_final_pose(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FINAL_POSE_DATA_1) then
                    int_final_pose(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_final_pose(63 downto 32));
                end if;
            end if;
        end if;
    end process;
